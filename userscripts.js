...
// @version      1.31
// @description  拾译 - 基于上下文语境的人工智能翻译引擎，随手拾译。程序Github开源：https://github.com/Bistutu/FluentRead，欢迎 star。
// @author       ThinkStu
// @match        *://*/*
...
if (window.trustedTypes && window.trustedTypes.createPolicy) {
    safeGlearn = window.trustedTypes.createPolicy("safeGlearn", {
...
const settingManager = {
  ...
  <div style="font-size: 1em;" xmlns="http://www.w3.org/1999/html">
    <label class="instant-setting-label">快捷键<select id="glearn-hotkey" class="instant-setting-common">${this.generateOptions(shortcutManager.hotkeyOptions, util.getValue('hotkey'))}</select></label>
    <label class="instant-setting-label">翻译源语言<select id="glearn-from" class="instant-setting-common">${this.generateOptions(langManager.from, langManager.getFrom())}</select></label>
    <label class="instant-setting-label">翻译目标语言<select id="glearn-to" class="instant-setting-common">${this.generateOptions(langManager.to, langManager.getTo())}</select></label>
    <label class="instant-setting-label">翻译服务<select id="glearn-model" class="instant-setting-select">${this.generateOptions(transModelName, util.getValue('model'))}</select></label>
    
    <!--支持 ollama 等自定义模型名称-->
    <label class="instant-setting-label" id="glearn-custom-type-label" style="display: none;">
    <span class="glearn-tooltip">自定义模型类型
            <span class="glearn-tooltiptext">
            请填写模型类型全称，如：gemma:7b、llama2:7b
            </span>
        </span>
    <input type="text" class="instant-setting-input" id="glearn-custom-type" value="${optionsManager.getOption(util.getValue('model'))}" ></label>
    
    <label class="instant-setting-label" id="glearn-option-label" style="display: none;">模型类型<select id="glearn-option" class="instant-setting-select"></select></label> 
    <!-- custom 输入框-->
    <label class="instant-setting-label" id="glearn-custom-label" style="display: none;">
        <span class="glearn-tooltip">自定义 GPT 地址
            <span class="glearn-tooltiptext">
        ...
        <input type="text" class="instant-setting-input" id="glearn-custom" value="${customGPT.getGPTUrl()}" >
    </label>
    <!-- 令牌区域 -->
    <label class="instant-setting-label" id="glearn-token-label" style="display: none;">token令牌<input type="text" class="instant-setting-input" id="glearn-token" value="" ></label>
    <label class="instant-setting-label" id="glearn-ak-label" style="display: none;">ak令牌<input type="text" class="instant-setting-input" id="glearn-ak" value="" ></label>
    <label class="instant-setting-label" id="glearn-sk-label" style="display: none;">sk令牌<input type="text" class="instant-setting-input" id="glearn-sk" value="" ></label>
    <!-- 添加的输入区域 -->
    <label class="instant-setting-label" id="glearn-system-label" style="display: none;">
        <span class="glearn-tooltip">system角色设定<span class="glearn-tooltiptext">模型角色设定，如：你是一名专业的翻译家</span></span>
        <textarea class="instant-setting-textarea" id="glearn-system-message">${chatMgs.getSystemMsg()}</textarea>
    </label>
    <label class="instant-setting-label" id="glearn-user-label" style="display: none;">
        <span class="glearn-tooltip">user消息模板<span class="glearn-tooltiptext">用户对话内容，如：请你翻译 Hello</br>注意：{{text}} 是你需要翻译的原文，不可缺少。</span></span>
        <textarea class="instant-setting-textarea" id="glearn-user-message">${chatMgs.getOriginUserMsg()}</textarea>
    </label>
  </div>`;
        Swal.fire({
                title: '设置中心',
                ...
                if (result.isConfirmed) {
                    let model = util.getElementValue('glearn-model');

                    // 0、设置自定义 GPT 地址
                    if ([transModel.openai, transModel.ollama].includes(model)) {
                        let ok = customGPT.setGPTUrl(model, util.getElementValue('glearn-custom'));
                        if (!ok) {
                            toast.fire({
                                icon: 'error',
                    ...
                    // 1、设置语言
                    util.setValue('from', util.getElementValue('glearn-from'));
                    util.setValue('to', util.getElementValue('glearn-to'));
                    // 2、设置快捷键
                    util.setValue('hotkey', util.getElementValue('glearn-hotkey'));
                    // 3、设置翻译服务
                    util.setValue('model', model);
                    // 4、设置模型类型
                    if (model === transModel.ollama) {
                        optionsManager.setOption(model, util.getElementValue('glearn-custom-type'));
                    } else {
                        optionsManager.setOption(model, util.getElementValue('glearn-option'));
                    }
                    // 5、存储 token
                    let token = util.getElementValue('glearn-token');
                    let ak = util.getElementValue('glearn-ak');
                    let sk = util.getElementValue('glearn-sk');
                    switch (model) {
                    ...
                    // 6、设置 chatGPT 消息模板
                    chatMgs.setSystemMsg(util.getElementValue('glearn-system-message'));
                    chatMgs.setUserMsg(util.getElementValue('glearn-user-message'));
        ...
        // 监听“翻译服务”选择框
        document.getElementById('glearn-model').addEventListener('change', e => {
    ...
    showHidden(model) {
        // label 是最外层的标签
        const tokenLabel = document.getElementById('glearn-token-label');
        const akLabel = document.getElementById('glearn-ak-label');
        const skLabel = document.getElementById('glearn-sk-label');

        const token = document.getElementById('glearn-token');
        const ak = document.getElementById('glearn-ak');
        const sk = document.getElementById('glearn-sk');
    ...
    setDisplayStyle(flex, none) {
        // 消息模版
        const systemMsgLabel = document.getElementById('glearn-system-label');
        const userMsgLabel = document.getElementById('glearn-user-label');
        // 自定义 GPT 地址框
        const customLabel = document.getElementById('glearn-custom-label');
        const optionLabel = document.getElementById('glearn-option-label');
        const customTypeLabel = document.getElementById('glearn-custom-type-label');
        ...
        document.getElementById('glearn-custom').value = customGPT.getGPTUrl(model);  // 设置自定义 GPT 地址
        ...
        if (model === transModel.ollama) {
            ...
            document.getElementById('glearn-custom-type').value = optionsManager.getCustomOption(model);
        } else {
            const optionSelect = document.getElementById('glearn-option');
        ...
        if (outerHTMLCache) {
            ...
            setTimeout(() => {  // 延迟 remove 转圈动画与替换文本
                ...
                if (fn) {
                    ...
                    node.outerHTML = safeGlearn ? safeGlearn.createHTML(outerHTMLCache) : outerHTMLCache;
                }
        ...
        transModelFn[model](origin).then(text => {
            ...
            if (isMachineTrans(model)) {
                ...
                if (fn) {
                    ...
                    node.outerHTML = safeGlearn ? safeGlearn.createHTML(text) : text;
                }
            } else {
                // LLM 翻译
                node.innerHTML = safeGlearn ? safeGlearn.createHTML(text) : text;
                newOuterHtml = node.outerHTML;
            }
...
function createLoadingSpinner(node, isCache) {
    ...
    spinner.className = 'loading-spinner-glearn';
...
function hasLoadingSpinner(node) {
    if (node.classList.contains('loading-spinner-glearn')) return true;
...
function initApplication() {
    ...
    .loading-spinner-glearn {border: 3px solid #f3f3f3;border-top: 3px solid blue;border-radius: 50%;width: 12px;height: 12px;animation: spin 1s linear infinite;display: inline-block;}
    ...
    /* 工具提示 */
    .glearn-tooltip { position: relative; display: inline-block; }
    .glearn-tooltip .glearn-tooltiptext { visibility: hidden; width: 25em; background-color: black; color: #fff; text-align: left; border-radius: 6px; padding: 5px; position: absolute; z-index: 1; bottom: 100%; left: 50%; margin-left: -60px; opacity: 0; transition: opacity 0.6s; font-size: 14px; }
    .glearn-tooltip:hover .glearn-tooltiptext { visibility: visible; opacity: 1; }
    ...
    @media (max-width: 600px) {
        ...
        .glearn-tooltip .glearn-tooltiptext {
            width: 18em; /* 调整工具提示宽度 */
